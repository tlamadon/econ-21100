...
<span class="label label-success">Question 6</span> Implement this in a function, report the estimate of $\gamma$ that it returns (please actually report a curve over $\gamma$). Use the following code to generate data. Comment on the result.
...
# solve for lambda
data[, Lb := ((p$rho*W1 - B)/a)^(1/(1+p$gamma))]
data[, Lb := pmin(pmax(Lb,0),1)] # bound it between 0 and 1

data[, D := rexp(p$n, rate = Lb)]
```

<span class="label label-success">Question 7 (bonus) </span> Repeat the fixed-effect approach for the case that we observe two sets of wage and unemployment duration. Specifically, we generate the data by the following code.
